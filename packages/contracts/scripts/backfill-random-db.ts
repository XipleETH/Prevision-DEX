...
async function main() {
  const oracle = process.env.RANDOM_ORACLE || process.env.ORACLE
  if (!oracle) throw new Error('Set RANDOM_ORACLE (or ORACLE) to the RandomOracle address')
  ...
  const flush = async () => {
    ...
    for (const p of payloads) {
      try {
        await axios.post(ingestUrl!, { secret: ingestSecret, chain, market, time: p.time, value: p.value }, { timeout: 10000 })
      } catch (e: any) {
    ...
    try {
      ...
      for (const l of logs) {
        try {
          ...
          if (time > 0 && value > 0) batch.push({ time, value })
  ...
  await flush()
  console.log('Random backfill completed for', chain)
}

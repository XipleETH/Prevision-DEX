...
export default async function handler(req: Request): Promise<Response> {
  ...
  const chain = String(searchParams.get('chain') || 'bsc-testnet').toLowerCase()
    ...
    const getTs = async (bn: number) => {
      ...
      if (!block) return NaN
  // Some RPCs return hex timestamp strings; normalize to number
      const ts = Number(block?.timestamp)
      return Number.isFinite(ts) ? ts : Number(block?.timestamp ? BigInt(block.timestamp) : NaN)
    }

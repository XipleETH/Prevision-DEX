...
export default async function handler(req: Request): Promise<Response> {
  try {
    ...
    // Query by score (timestamp seconds). Upstash supports byScore in zrange
  const arr = await redis.zrange<[string | number]>(ticksKey, min, max, { byScore: true, withScores: true, offset: 0, count: limit })
    ...
    for (const p of points) {
      ...
      // normalize to 1e-8 precision to avoid fp noise
...
function parseTimeToSec(v: string): number {
  if (!v) return NaN
  // If numeric, assume seconds if looks like seconds; if looks like ms, convert
  if (/^\d+$/.test(v)) {
    const n = Number(v)
    if (n > 3_000_000_000) return Math.floor(n / 1000) // ms
    return Math.floor(n)
  }
  const d = new Date(v)
